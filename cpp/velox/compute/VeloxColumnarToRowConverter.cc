...
void VeloxColumnarToRowConverter::ResumeVeloxVector() {
  ...
  for (int col_idx = 0; col_idx < num_cols_; col_idx++) {
    ...
    }else
    {
      ...
      velox::VectorPtr copy = 
        velox::BaseVector::create(child->type(), child->size(), child->pool());
...
arrow::Status VeloxColumnarToRowConverter::Write() {
  ...
  for (auto col_index = 0; col_index < num_cols_; col_index++) {
    ...
    if (arrow::bit_width(typevec[col_index]) > 1 || typevec[col_index] == arrow::StringType::type_id || typevec[col_index] == arrow::BinaryType::type_id) {
