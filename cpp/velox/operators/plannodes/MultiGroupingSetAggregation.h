...
/// Builds intermediate aggregate states for a grouping-set derivation tree.
///
/// The root consumes source columns and constructs partial states. Every other
/// set merges its parent's emitted states. A prefix ROLLUP is therefore a
/// cascade from the finest grain to the grand total. Each node owns its drain
/// iterator, allowing parent drains to pause safely while a child drains.
class MultiGroupingSetAggregation : public Operator {
 public:
  ...
  /// Reports only bytes reclaim() can release immediately: unused explicit
  /// reservation plus retained allocations from empty grouping sets.
 ...
 private:
  ...
  enum class FlushReason {
    kNone,
    /// A raw-input root crossed the ordinary partial-aggregation
    /// input/distinct threshold. Drain its live table before switching it to
    /// raw-to-intermediate conversion mode.
    kEarlyAbandon,
  ...
  struct NodeState {
    ...
    /// ROW(<active keys>, acc1..accm), used for GroupingSet output and as input
    /// to descendant GroupingSets.
    RowTypePtr naturalType;

    /// True when this node is a lattice root that consumes the operator's raw
    /// source row rather than intermediate state.
    bool rawInputRoot{false};
    ...
    /// Null for a descendant whose dynamically abandoned table has been
    /// drained and dropped. An abandoned raw root retains this object as a
    /// raw-to-intermediate converter.
    ...
    /// Once dynamically abandoned a descendant drops its table and becomes a
    /// pure projection. A raw-input root retains its GroupingSet in
    /// converter mode and materializes one intermediate state per raw row.
    bool dynamicallyAbandoned{false};

    /// True when a raw-input root used HashAggregation's per-batch
    /// input/distinct test instead of the conservative pressure-cycle policy.
    bool earlyAbandoned{false};

    /// True when a raw-input root matched HashAggregation's secondary valve:
    /// more than 40% of a full-size table survived aggregation after the node
    /// had already reached its maximum extended memory budget.
    bool maxMemoryAbandoned{false};

    /// Number of successful node-local partial-aggregation budget increases.
    int32_t budgetGrowths{0};

    /// Forwarded state rows or converted raw rows after dynamic abandonment.
  ...
  /// A raw root with no live table. Its retained GroupingSet converts raw rows
  /// to type-correct intermediate states and is not flushable or reclaimable.
  static bool isRawConverter(const NodeState& node) {
    return node.rawInputRoot && node.dynamicallyAbandoned;
  }
  ...
  /// Returns the operator source unchanged for a raw root. Otherwise reshapes a
  /// batch of the parent's naturalType into node
  /// `index`'s naturalType. Reshaping is pure column selection: one reference
  /// copy per column, no data movement, arbitrary lattice.
  ...
  /// Feeds one input-shaped batch into node `index`: source-shaped for a raw
  /// root, naturalType-shaped otherwise. Honors the abandon valve
  /// and recurses into children when the node is a pass-through/converter. When
  /// supplied, touchedTableNodes marks every live table actually fed through
  /// that recursion.
  ...
  /// Records one irreversible adaptive-abandon decision and its evidence.
  void markDynamicallyAbandoned(int32_t index, int64_t inputRows, int64_t outputRows, bool early, bool atMaxMemory);

  /// Mirrors HashAggregation's early partial-aggregation valve for raw lattice
  /// roots. The check uses only the current table cycle, whose input counter is
  /// reset after every drain.
  bool shouldAbandonRawRootEarly(int32_t index) const;
  ...
  /// At a safe scheduling boundary, arms one effectiveness or local-pressure
  /// drain or, when the shared target is exceeded, the largest eligible table.
  /// Returns true when a drain was started.
  ...
  /// HashAggregation's fixed output/input cutoff after reaching the maximum
  /// extended memory budget.
  static constexpr int32_t kRawMaxMemoryAbandonMinPct = 40;
  ...
  /// Pass-through rows and parked flush output. Bounded by
  ...
  /// Node-row operations on abandon lanes. A row forwarded or converted
  /// through two levels is intentionally counted twice.
  int64_t dynamicAbandonRows_{0};
  int64_t rawConvertedRows_{0};
