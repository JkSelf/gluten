...
void MultiGroupingSetAggregation::initialize() {
  ...
  plan_ = buildDerivationPlan(masks);

  for (auto i = 0; i < numSets; ++i) {
    nodes_[i].rawInputRoot = plan_.parent[i] == kRawInputParent;
  }
  ...
  // Column selections. A raw root receives the source row directly. Every
  // descendant receives batches of its own naturalType, produced by derive()
  // from its parent's naturalType.
  for (auto i = 0; i < numSets; ++i) {
    auto& node = nodes_[i];
    const auto parent = plan_.parent[i];
    if (parent == kRawInputParent) {
      // The raw root consumes the source RowVector directly. Its grouping keys
      // and aggregate arguments are resolved by field name when the GroupingSet
      // and AggregateInfos are built.
      continue;
    } else {
  ...
  // Every grouping set starts with a table. An ineffective raw root or
  // descendant may abandon its table adaptively after it has been drained.
  for (auto i = 0; i < numSets; ++i) {
    auto& node = nodes_[i];
  ...
  addRuntimeStat("gsagg.pressureAbandonMinEvidenceCycles", RuntimeCounter(kAbandonMinEvidenceCycles));
  addRuntimeStat("gsagg.rawEarlyAbandonMinEvidenceCycles", RuntimeCounter(1));
  addRuntimeStat("gsagg.rawMaxMemoryAbandonMinPct", RuntimeCounter(kRawMaxMemoryAbandonMinPct));
  int32_t numRoots{0};
  for (auto i = 0; i < numSets; ++i) {
...
RowTypePtr MultiGroupingSetAggregation::makeNaturalType(const NodeState& node) const {
  ...
  for (auto channel : node.activeKeys) {
    names.push_back(node_->groupingKeys()[channel]->name());
    types.push_back(node_->groupingKeys()[channel]->type());
  }
...
std::vector<AggregateInfo> MultiGroupingSetAggregation::makeAggregateInfos(int32_t index) {
  ...
  // Extracted accumulators always sit immediately after this node's active
  // keys. Descendants read those same channels on input. Raw roots instead
  // resolve aggregate call arguments against the source row.
  const auto accBase = node.numActive();
  const auto& groupingSetInputType = node.rawInputRoot ? node_->inputType() : node.naturalType;
  ...
  for (auto i = 0; i < numAggregates_; ++i) {
    ...
    // Flush and lattice derivation require decomposable aggregates.
    VELOX_CHECK_NULL(
        aggregate.mask,
    ...
    if (node.rawInputRoot) {
      for (const auto& input : aggregate.call->inputs()) {
        if (const auto field = std::dynamic_pointer_cast<const core::FieldAccessTypedExpr>(input)) {
          info.inputs.push_back(groupingSetInputType->getChildIdx(field->name()));
          info.constantInputs.push_back(nullptr);
        } else if (const auto constant = std::dynamic_pointer_cast<const core::ConstantTypedExpr>(input)) {
          info.inputs.push_back(kConstantChannel);
          info.constantInputs.push_back(constant->toConstantVector(pool()));
        } else {
          VELOX_FAIL("Raw aggregate {} input must be a field access or constant: {}", name, input->toString());
        }
      }
    } else {
    ...
    // kPartial makes the function emit intermediate state via
    // extractAccumulators. GroupingSet's isRawInput flag selects addRawInput
    // for roots and addIntermediateResults for descendants.
    // Aggregate::create always resolves the signature from raw argument types,
    // regardless of that input mode.
    ...
    if (node.rawInputRoot && !info.constantInputs.empty()) {
...
void MultiGroupingSetAggregation::makeGroupingSet(int32_t index) {
  ...
  if (node.rawInputRoot) {
    for (const auto key : node.activeKeys) {
      keyChannels.push_back(node_->inputType()->getChildIdx(node_->groupingKeys()[key]->name()));
    }
  } else {
  ...
  const auto& groupingSetInputType = node.rawInputRoot ? node_->inputType() : node.naturalType;
  auto hashers = createVectorHashers(groupingSetInputType, keyChannels);

  node.groupingSet = std::make_unique<GroupingSet>(
      groupingSetInputType,
      std::move(hashers),
...
RowVectorPtr MultiGroupingSetAggregation::derive(int32_t index, const RowVectorPtr& source) const {
  ...
  if (node.rawInputRoot) {
    VELOX_DCHECK(
        source->type()->equivalent(*node_->inputType()), "Raw lattice root must receive the operator source row");
    return source;
  }
...
RowVectorPtr MultiGroupingSetAggregation::project(int32_t index, const RowVectorPtr& natural) const {
  ...
  for (auto j = 0; j < numKeys_; ++j) {
    if ((node.activeKeysMask & (GroupingSetMask{1} << j)) != 0) {
      ...
      const auto& keyType = node_->groupingKeys()[j]->type();
      children.push_back(BaseVector::createNullConstant(keyType, size, pool()));
    }
  }
...
void MultiGroupingSetAggregation::feedNode(
  ...
  if (isRawConverter(node)) {
    VELOX_CHECK_NOT_NULL(node.groupingSet);
    if (node.output != nullptr) {
      VectorPtr reusable = std::move(node.output);
      BaseVector::prepareForReuse(reusable, natural->size());
      node.output = std::static_pointer_cast<RowVector>(reusable);
    } else {
      node.output = std::static_pointer_cast<RowVector>(BaseVector::create(node.naturalType, natural->size(), pool()));
    }

    // An abandoned raw root cannot forward the source row: descendants and the
    // operator output consume aggregate states. GroupingSet's converter
    // materializes exactly one state row per raw row, including constants,
    // null masks, and aggregate-specific intermediate layouts.
    node.groupingSet->toIntermediate(natural, node.output);
    VELOX_CHECK_NOT_NULL(node.output);
    VELOX_CHECK_EQ(node.output->size(), natural->size());

    node.dynamicAbandonRows += natural->size();
    dynamicAbandonRows_ += natural->size();
    rawConvertedRows_ += natural->size();
    node.numOutputRows += natural->size();
    node.totalOutputRows += natural->size();
    outputQueue_.push_back(project(index, node.output));
    feedChildren(index, node.output, touchedTableNodes);
    return;
  }

  if (node.groupingSet == nullptr) {
    // A dynamically abandoned descendant is an identity operation over
    // intermediate states.
  ...
  // Descendant abandonment drops the GroupingSet before completeNodeDrain()
  // returns. Raw-root abandonment retains it in converter mode, handled above.
  VELOX_DCHECK(!node.dynamicallyAbandoned);
  ...
  // Raw roots are checked at the post-batch scheduling boundary. Derived and
  // descendants use completed pressure-drain evidence instead.
}

void MultiGroupingSetAggregation::markDynamicallyAbandoned(
    int32_t index,
    int64_t inputRows,
    int64_t outputRows,
    bool early,
  ...
  node.earlyAbandoned = early;
  node.maxMemoryAbandoned = atMaxMemory;
  const auto pct =
      static_cast<int64_t>(static_cast<FlushableBytes>(outputRows) * 100 / static_cast<FlushableBytes>(inputRows));
  const auto prefix = fmt::format("gsagg.set{}", index);
  addRuntimeStat(prefix + ".abandoned", RuntimeCounter(1));
  addRuntimeStat(prefix + ".abandonInputRows", RuntimeCounter(inputRows));
  addRuntimeStat(prefix + ".abandonOutputRows", RuntimeCounter(outputRows));
  addRuntimeStat(prefix + ".abandonPct", RuntimeCounter(pct));
}
...
void MultiGroupingSetAggregation::maybeAbandonAfterDrain(int32_t index) {
  ...
  if (node.reason == FlushReason::kEarlyAbandon) {
    VELOX_DCHECK(node.rawInputRoot);
    VELOX_DCHECK_GT(node.numInputRows, abandonPartialAggregationMinRows_);
    markDynamicallyAbandoned(index, node.numInputRows, node.numOutputRows, /*early=*/true, /*atMaxMemory=*/false);
    return;
  }
  ...
  // Match HashAggregation's second raw-input valve. Once the table has
  // reached the maximum extended budget, retaining it is not worthwhile if
  // more than 40% of its input still has to be emitted. This is deliberately
  // limited to local full-table drains; a shared operator-target drain does
  // not prove that this node exhausted its own useful memory budget.
  if (node.rawInputRoot && (node.reason == FlushReason::kSoftBudget || node.reason == FlushReason::kHardCap) &&
      node.maxPartialBytes >= maxExtendedPartialAggregationMemoryUsage_) {
    if (static_cast<FlushableBytes>(node.numOutputRows) * 100 >
        static_cast<FlushableBytes>(node.numInputRows) * kRawMaxMemoryAbandonMinPct) {
      markDynamicallyAbandoned(index, node.numInputRows, node.numOutputRows, /*early=*/false, /*atMaxMemory=*/true);
      return;
    }
  }
  ...
  markDynamicallyAbandoned(
      index,
      node.abandonWindowInRows,
      node.abandonWindowOutRows,
      /*early=*/false,
      /*atMaxMemory=*/false);
}

bool MultiGroupingSetAggregation::shouldAbandonRawRootEarly(int32_t index) const {
  const auto& node = nodes_[index];
  if (!node.rawInputRoot || node.numActive() == 0 || node.groupingSet == nullptr || node.draining ||
      node.dynamicallyAbandoned || isRawConverter(node) || node.numInputRows <= abandonPartialAggregationMinRows_) {
    return false;
  }

  // This is the same per-batch policy as HashAggregation: once enough rows
  // have been observed, stop spending CPU and memory when most rows created a
  // distinct finest-level group. Use a wide product at the reporting boundary
  // so large partitions cannot overflow the percentage calculation.
  const auto distinctRows = node.groupingSet->numDistinct();
  const auto distinctPct = static_cast<int64_t>(
      static_cast<FlushableBytes>(distinctRows) * 100 / static_cast<FlushableBytes>(node.numInputRows));
  return distinctPct >= abandonPartialAggregationMinPct_;
}
...
MultiGroupingSetAggregation::FlushableBytes MultiGroupingSetAggregation::flushableNodeBytes() const {
  ...
  for (const auto& node : nodes_) {
    if (node.groupingSet != nullptr && !isRawConverter(node)) {
...
MultiGroupingSetAggregation::FlushReason MultiGroupingSetAggregation::localFlushReason(
  ...
  auto& node = nodes_[index];
  if (node.groupingSet == nullptr || node.draining || isRawConverter(node)) {
...
bool MultiGroupingSetAggregation::exceedsHardCap(int32_t index) const {
  const auto& node = nodes_[index];
  if (node.groupingSet == nullptr || node.draining || node.dynamicallyAbandoned || isRawConverter(node)) {
    return false;
  }
...
bool MultiGroupingSetAggregation::maybeStartPressureDrain() {
  ...
  // A raw root sees the same unexpanded source rows as ordinary partial
  // HashAggregation. Apply its per-batch effectiveness check before waiting
  // for memory pressure. The table must be drained before GroupingSet can enter
  // raw-to-intermediate converter mode.
  for (auto i : plan_.order) {
    if (shouldAbandonRawRootEarly(i)) {
      recordPressureSample();
      pushDrain(i, FlushReason::kEarlyAbandon);
      return true;
    }
  }
  ...
  if (candidate == -1) {
    ...
    for (auto i : plan_.order) {
      auto& node = nodes_[i];
      if (node.groupingSet == nullptr || node.draining || isRawConverter(node) || nodeHasOutput(i) ||
...
RowVectorPtr MultiGroupingSetAggregation::getOutput() {
  // Emit queued pass-through rows first.
...
void MultiGroupingSetAggregation::completeNodeDrain(int32_t index) {
  ...
  if (node.dynamicallyAbandoned && node.rawInputRoot) {
    // The drained table is empty. Retain the raw GroupingSet as a type-safe
    // raw-to-intermediate converter for all subsequent batches.
    node.groupingSet->abandonPartialAggregation();
    // Unlike HashAggregation, this operator's pool is shared by every
    // grouping-set table. Keep unused reservation available to live
    // descendants whose local budgets may already have grown.
  } else {
    // Pinned Velox frees a hash table when getOutput() reaches exhaustion. Make
    // that contract explicit instead of pretending a slot array can be
    // retained. A global set has no hash table; reset its reusable row and
    // external aggregate state after its intermediate result has been emitted.
    if (node.numActive() == 0) {
      node.groupingSet->resetGlobalAggregation();
    } else {
      node.groupingSet->resetTable(/*freeTable=*/true);
    ...
    if (node.dynamicallyAbandoned) {
      // An abandoned descendant is an identity projection from here on. Drop the
      // GroupingSet and its reusable output buffer so pressure scans stop
      // reporting the table and the dead buffer does not remain pinned until
      // operator close. Any projected or derived vectors retain their own
      // shared ownership of the buffer's children.
...
bool MultiGroupingSetAggregation::reclaimableBytes(uint64_t& reclaimableBytes) const {
  // A successful maybeGrowBudget() can leave an explicit minimum reservation
  // after its table is drained. Include that immediately releasable headroom;
  // otherwise arbitration sees a zero estimate and never calls reclaim() to
  // clear the minimum reservation. Live accumulator state is not reclaimable.
  reclaimableBytes = 0;
  if (nonReclaimableSection_) {
    return false;
  }
  const auto releasableReservation = pool()->releasableReservation();
  if (releasableReservation > 0) {
    reclaimableBytes = static_cast<uint64_t>(releasableReservation);
  }
...
void MultiGroupingSetAggregation::recordNodeStats(int32_t index) {
  ...
  if (node.reason == FlushReason::kEarlyAbandon || node.reason == FlushReason::kSoftBudget ||
      node.reason == FlushReason::kHardCap || node.reason == FlushReason::kOperatorTarget) {
    // Reuse the standard aggregation metric for non-final partial-drain rows so
    // existing metric consumers can surface pressure activity without a
    // dedicated transport field. End-of-input drains are deliberately excluded.
...
void MultiGroupingSetAggregation::recordFinalStats() {
  ...
  int32_t earlyAbandonedSets{0};
  int32_t maxMemoryAbandonedSets{0};
  for (auto i = 0; i < static_cast<int32_t>(nodes_.size()); ++i) {
    const auto& node = nodes_[i];
    const auto prefix = fmt::format("gsagg.set{}", i);

    // recordNodeStats() reports table-backed cycles as deltas. Dynamically
    // abandoned nodes may forward or convert more rows after their last drain,
    // so close the delta accounting for that phase here. The explicit totals
    // provide a single authoritative lifetime value as well.
    if (node.dynamicallyAbandoned) {
      addRuntimeStat(prefix + ".inputRows", RuntimeCounter(node.numInputRows));
      addRuntimeStat(prefix + ".outputRows", RuntimeCounter(node.numOutputRows));
    }
    ...
    addRuntimeStat(prefix + ".earlyAbandoned", RuntimeCounter(node.earlyAbandoned ? 1 : 0));
    addRuntimeStat(prefix + ".maxMemoryAbandoned", RuntimeCounter(node.maxMemoryAbandoned ? 1 : 0));
    addRuntimeStat(prefix + ".budgetGrowths", RuntimeCounter(node.budgetGrowths));
    dynamicallyAbandonedSets += node.dynamicallyAbandoned ? 1 : 0;
    earlyAbandonedSets += node.earlyAbandoned ? 1 : 0;
    maxMemoryAbandonedSets += node.maxMemoryAbandoned ? 1 : 0;
  }

  addRuntimeStat("gsagg.dynamicAbandonRows", RuntimeCounter(dynamicAbandonRows_));
  addRuntimeStat("gsagg.rawConvertedRows", RuntimeCounter(rawConvertedRows_));
  // Same standard metric used by HashAggregation. On this operator it counts
  // rows forwarded or converted after the adaptive per-set table was
  // abandoned.
  ...
  addRuntimeStat("gsagg.earlyAbandonedSets", RuntimeCounter(earlyAbandonedSets));
  addRuntimeStat("gsagg.maxMemoryAbandonedSets", RuntimeCounter(maxMemoryAbandonedSets));
