...
import org.apache.gluten.extension.BroadcastJoinContextTag
...
class VeloxSparkPlanExecApi extends SparkPlanExecApi with Logging {
  ...
  override def createBroadcastRelation(
    ...
    if (shouldBuildOnDriver) {
      ...
      val joinContextOpt: Option[org.apache.gluten.extension.BroadcastJoinContextInfo] =
      ...
      joinContextOpt match {
        case Some(joinContext) =>
          ...
          val buildContext = org.apache.gluten.execution.BroadcastHashJoinContext(
            ...
            isExistenceJoin = joinContext.joinType
              .isInstanceOf[org.apache.spark.sql.catalyst.plans.ExistenceJoin],
          ...
          try {
            ...
            val (serializedHashTable, safeMode) = columnarRelation match {
              case rel: org.apache.spark.sql.execution.ColumnarBuildSideRelation =>
                (
                  org.apache.gluten.execution.VeloxBroadcastBuildSideCache
                    .buildAndSerializeOnDriverInBroadcastExchange(rel, buildContext),
                  rel.safeBroadcastMode
                )
              case rel: org.apache.spark.sql.execution.unsafe.UnsafeColumnarBuildSideRelation =>
                (
                  org.apache.gluten.execution.VeloxBroadcastBuildSideCache
                    .buildAndSerializeOnDriverInBroadcastExchange(rel, buildContext),
            ...
            org.apache.spark.sql.execution.SerializedHashTableBroadcastRelation(
              serializedHashTable,
              safeMode,
              newOutput,
