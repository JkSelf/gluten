 ...
 * (`_gluten_rw_selfjoin_neq_min`) as a soft indicator that the rule fired. Its absence is not
...
class RewriteSelfJoinInequalityToAggregateSuite extends WholeStageTransformerSuite {
  ...
  private val NeqMinAlias = "_gluten_rw_selfjoin_neq_min"

  private def ruleFired(plan: LogicalPlan): Boolean =
    plan.exists {
      p =>
        p.expressions.exists(_.exists {
          case a: Alias if a.name == NeqMinAlias => true
          case _ => false
        })
    }
  ...
  // ==================== Pattern A3: self-join split by ReorderJoin ====================

  /**
   * `SJ` is the self-joined relation -- k in {1,3} have >= 2 distinct non-null v, k in {2,4} do
   * not. `TH` plays Q95's `web_returns`: the third relation that `ReorderJoin` fuses with one copy
   * of `SJ`, stranding the self-join predicates on the outer join.
   */
  private def setupSplitTables(): Unit = {
    spark.sql(
      """CREATE OR REPLACE TEMP VIEW SJ AS SELECT * FROM VALUES
        |  (1, 10), (1, 20),
        |  (2, 30),
        |  (3, 40), (3, 50),
        |  (4, 60), (4, CAST(NULL AS INT))
        |AS SJ(k, v)""".stripMargin)
    spark.sql(
      """CREATE OR REPLACE TEMP VIEW TH AS SELECT * FROM VALUES
        |  (1), (2) AS TH(k)""".stripMargin)
  }

  test("Pattern A3: three-way join strands the self-join predicates on the outer join") {
    setupSplitTables()
    // `FROM TH t, SJ s1, SJ s2` is reordered to `(TH |x| s1) |x| s2`, so `s1.k = s2.k AND
    // s1.v <> s2.v` ends up on the top join and the self-join is no longer a subtree.
    val sql =
      """SELECT k FROM SJ o WHERE k IN (
        |  SELECT t.k FROM TH t, SJ s1, SJ s2
        |  WHERE t.k = s1.k AND s1.k = s2.k AND s1.v <> s2.v)""".stripMargin
    val (on, off) = runBoth(sql)
    assert(on == off, s"Pattern A3 rewrite ON $on != OFF $off")
    // Qualifying k from the self-join is {1,3}; TH restricts it to {1}.
    assert(on == Set(Row(1)), s"expected {1}, got $on")
    // Positive plan assertion, unlike most of this suite: A3 exists purely to fire on this shape,
    // and `on == off` would pass even if it silently bailed out.
    withSQLConf("spark.gluten.sql.rewrite.selfJoinInequality" -> "true") {
      val plan = spark.sql(sql).queryExecution.optimizedPlan
      assert(ruleFired(plan), s"A3 did not fire on the split self-join shape:\n$plan")
    }
  }

  test("Pattern A3: top-level Project selecting the surviving copy's equi key is remapped") {
    setupSplitTables()
    // Selects `s1.k` rather than `t.k`, so the subquery's Project reaches into the copy A3
    // deletes and has to be redirected onto the Aggregate's grouping output.
    val sql =
      """SELECT k FROM SJ o WHERE k IN (
        |  SELECT s1.k FROM TH t, SJ s1, SJ s2
        |  WHERE t.k = s1.k AND s1.k = s2.k AND s1.v <> s2.v)""".stripMargin
    val (on, off) = runBoth(sql)
    assert(on == off, s"Pattern A3 equi-key remap rewrite ON $on != OFF $off")
    assert(on == Set(Row(1)), s"expected {1}, got $on")
    withSQLConf("spark.gluten.sql.rewrite.selfJoinInequality" -> "true") {
      val plan = spark.sql(sql).queryExecution.optimizedPlan
      assert(ruleFired(plan), s"A3 did not fire when the Project selects the equi key:\n$plan")
    }
  }

  test("Pattern A3: inner join touching a non-equi column of the deleted copy bails out") {
    setupSplitTables()
    spark.sql(
      """CREATE OR REPLACE TEMP VIEW TH2 AS SELECT * FROM VALUES
        |  (1, 10), (2, 30), (3, 40) AS TH2(k, x)""".stripMargin)
    // `t.x = s1.v` lands on the inner join and constrains s1.v, which cannot survive folding
    // s1's rows into groups. A3 must refuse rather than drop the predicate.
    val sql =
      """SELECT k FROM SJ o WHERE k IN (
        |  SELECT t.k FROM TH2 t, SJ s1, SJ s2
        |  WHERE t.k = s1.k AND t.x = s1.v AND s1.k = s2.k AND s1.v <> s2.v)""".stripMargin
    val (on, off) = runBoth(sql)
    assert(on == off, s"bail-out path changed results: ON=$on OFF=$off")
    // t=(1,10) picks s1=(1,10) and s2=(1,20); t=(3,40) picks s1=(3,40) and s2=(3,50);
    // t=(2,30) finds no s2 with a different v. So the subquery yields {1,3}.
    assert(on == Set(Row(1), Row(3)), s"expected {1,3}, got $on")
    val plan = spark.sql(sql).queryExecution.optimizedPlan
    assert(!ruleFired(plan), s"A3 must not fire when the inner join constrains s1.v:\n$plan")
  }

  test("Pattern A' and A3 together: both occurrences of one self-join rewrite (Q95 shape)") {
    setupSplitTables()
    // Q95's structure: the same self-join appears once directly under an IN and once fused with
    // a third relation. Both must rewrite -- that is what lets ReuseExchange collapse the two
    // identical Aggregate subtrees into a single scan and shuffle at execution time.
    val sql =
      """SELECT k FROM SJ o
        |WHERE k IN (SELECT s1.k FROM SJ s1, SJ s2
        |            WHERE s1.k = s2.k AND s1.v <> s2.v)
        |  AND k IN (SELECT t.k FROM TH t, SJ s1, SJ s2
        |            WHERE t.k = s1.k AND s1.k = s2.k AND s1.v <> s2.v)""".stripMargin
    val (on, off) = runBoth(sql)
    ...
    withSQLConf("spark.gluten.sql.rewrite.selfJoinInequality" -> "true") {
      val plan = spark.sql(sql).queryExecution.optimizedPlan
      val aggregates = plan.collect { case a: Aggregate => a }
      assert(
        aggregates.size >= 2,
        s"expected both self-joins rewritten to Aggregates, found ${aggregates.size}:\n$plan")
    }
  }
  ...
  test("float edge cases: NaN and -0.0 in the inequality column stay equivalent") {
    // Guards the MIN(v) <> MAX(v) form specifically. It compares only the group's two
    // ordering extremes, so it must not disagree with the pairwise `<>` on the values where
    // Spark's ordering and its `EqualTo` part ways.
    //   k=1: {0.0, -0.0}   -- ordering separates them, EqualTo may not
    //   k=2: {NaN, NaN}    -- ordering-equal, and Spark defines NaN = NaN as true
    //   k=3: {NaN, 1.0}    -- must match
    //   k=4: {2.0, 2.0}    -- must not match
    //   k=5: {NaN, NULL}   -- only one non-null value, must not match
    spark.sql(
      """CREATE OR REPLACE TEMP VIEW TF AS SELECT * FROM VALUES
        |  (1, CAST(0.0 AS DOUBLE)), (1, CAST(-0.0 AS DOUBLE)),
        |  (2, CAST('NaN' AS DOUBLE)), (2, CAST('NaN' AS DOUBLE)),
        |  (3, CAST('NaN' AS DOUBLE)), (3, CAST(1.0 AS DOUBLE)),
        |  (4, CAST(2.0 AS DOUBLE)), (4, CAST(2.0 AS DOUBLE)),
        |  (5, CAST('NaN' AS DOUBLE)), (5, CAST(NULL AS DOUBLE))
        |AS TF(k, v)""".stripMargin)
    val sql =
      """SELECT k FROM TF outer_t WHERE k IN (
        |  SELECT s1.k FROM TF s1, TF s2
        |  WHERE s1.k = s2.k AND s1.v <> s2.v)""".stripMargin
    val (on, off) = runBoth(sql)
    assert(on == off, s"float inequality semantics diverge: ON=$on OFF=$off")
    // k=1 is deliberately not pinned: whether `-0.0 <> 0.0` holds is Spark's call, and both
    // forms evaluate the very same `Not(EqualTo(-0.0, 0.0))`, so `on == off` is the assertion.
    assert(on.contains(Row(3)), s"k=3 ({NaN, 1.0}) must match: $on")
    assert(!on.contains(Row(2)), s"k=2 ({NaN, NaN}) must not match: $on")
    assert(!on.contains(Row(4)), s"k=4 ({2.0, 2.0}) must not match: $on")
    assert(!on.contains(Row(5)), s"k=5 ({NaN, NULL}) must not match: $on")
    // DoubleType is orderable, so the rewrite is expected to fire rather than bail out.
    withSQLConf("spark.gluten.sql.rewrite.selfJoinInequality" -> "true") {
      val plan = spark.sql(sql).queryExecution.optimizedPlan
      assert(ruleFired(plan), s"rule should fire on a DoubleType inequality column:\n$plan")
    }
  }
  ...
  test("IS DISTINCT FROM: NULL-safe inequality preserves original semantics") {
    ...
    // MIN/MAX, because MIN/MAX ignore NULL.
    val sql =
      """SELECT k FROM T ws1 WHERE EXISTS (
  ...
  test("IsNotNull on a non-join column: results unchanged and rule bails out") {
    ...
    // `IsNotNull(w)`, the rewrite `GROUP BY k HAVING MIN(v) <> MAX(v)` would
